...
def getRadialPoints(radius, number_points):
    points = []
    r=radius
    a=0
    #choose random lat and long

    while a < number_points:
        # print(a)
        lat = np.random.uniform(-2*np.pi,2*np.pi)
        long = np.random.uniform(-2*np.pi,2*np.pi)
        # lat = 2*np.random.random_sample()
        # long = 2*np.random.random_sample()
...
if __name__ == '__main__':
    ...
    points = getRadialPoints(2, 1000)
    print(points)
    function([0,0,0],points)

    function([0,0,0], 2*np.random.rand(1000,3)-1)
